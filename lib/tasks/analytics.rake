...
task :analytics => :environment do
  ...
  data[:place_url]           = String.new
    Meetup.count(group: [:place,:url]).each_pair do |place_url, num|
      data[:place_url] += "#{place_url[0]},#{place_url[1]},#{num}\n"
    end
  data[:total_meetup_attendee] = UserMeetupPermission.all.size
